...
const Home: React.FC = () => {
    ...
    // useEffect(() => {
    //     if (isLoggedIn) {
    //         fetch('${process.env.IP_ADDRESS}:3001/getAllData', {
    //             method: 'POST',
    ...
    useEffect(() => {
        if (isLoggedIn) {
            fetch(`${process.env.IP_ADDRESS}:3001/hostsByEmail?email=${encodeURIComponent(userEmail)}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
            })
            .then(response => {
            ...
            const detailedGroups = await Promise.all(
                ruleGroupIds.map(async (id: number) => {
                    const [rules, remarks] = await Promise.all([
                        fetch(`${process.env.IP_ADDRESS}:3001/getRules?ruleGroupId=${id}`).then(r => r.json()),
                        fetch(`${process.env.IP_ADDRESS}:3001/getRemarks?ruleGroupId=${id}`).then(r => r.json())
                    ]);
                    return {
                        id,
